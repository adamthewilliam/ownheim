...
//
//   - Namespace imports (`import * as ns from '@ownheim/core'`) are
...
//   - Re-exports (`export { logger } from '@ownheim/core'`) are left
...
function transformSourceFile(sourceFile: SourceFile, resolvedOwner: string): string {
  ...
  // Collect factory-bound bindings from all `@ownheim/core[/sub]` imports.
  // Each binding becomes a `const <local> = createX(<owner literal>);`
  // initializer; we also add a sibling `import { createX } from
  // '@ownheim/core/<folder>/createX';` declaration for each unique factory.
...
function addFactoryImports(sourceFile: SourceFile, factories: ReadonlySet<string>): void {
  // Insert one `import { createX } from '@ownheim/core/<folder>/createX';`
